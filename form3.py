...
def process_form3(filepath, form1_filepath, progress_var, root, on_form3_done):
    ...
    code_col = get_column_letter(df_form3.columns.get_loc("Код товара") + 1)
    start_quantity_col = get_column_letter(df_form3.columns.get_loc("Количество") + 1)
    quantity_col = get_column_letter(df_form3.columns.get_loc("Количество с учетом единицы измерения") + 1)
    unit_col = get_column_letter(df_form3.columns.get_loc("ед. изм.") + 1)
    volume_col = get_column_letter(df_form3.columns.get_loc("Объем единицы, м3") + 1)
    final_volume_col = get_column_letter(df_form3.columns.get_loc("Итоговый объем, м3") + 1)
    date_col = get_column_letter(df_form3.columns.get_loc("Приведенная дата") + 1)

    # Заполняем формулы
    for row in range(2, len(df_form3) + 2):
        ...
        progress_var.set(f"Обработка данных: {row - 1} / {len(df_form3)} строк")
        root.update()
    ...
    unique_dates = df_form3["Приведенная дата"].dropna().unique()
    ...
    for idx, date in enumerate(sorted(unique_dates), start=2):
        ...
        date_cell = f"A{idx}"
        sheet_svod[f"B{idx}"] = f"=SUMIF('{sheet.title}'!{date_col}:{date_col}, {date_cell}, '{sheet.title}'!{final_volume_col}:{final_volume_col})"

        # Подсчет количества строк для текущей даты
        row_count = df_form3[df_form3["Приведенная дата"] == date].shape[0]
        sheet_svod[f"C{idx}"].value = row_count

        # Формула для суммы количества с учетом единицы измерения
        sheet_svod[f"D{idx}"] = f"=SUMIF('{sheet.title}'!{date_col}:{date_col}, {date_cell}, '{sheet.title}'!{quantity_col}:{quantity_col})"

        # Подсчет количества уникальных документов для текущей даты
        document_count = df_form3[df_form3["Приведенная дата"] == date]["Номер документа"].nunique()
