...
def improve_data_quality():
    ...
    if 'Объем единицы после аппроксимации, м3' in df.columns:
        df['Объем единицы итоговый, м3'] = df['Объем единицы после аппроксимации, м3']
    else:
        if 'Объем единицы после обработки выбросов, м3' in df.columns:
            df['Объем единицы итоговый, м3'] = df['Объем единицы после обработки выбросов, м3']
        else:
            df['Объем единицы итоговый, м3'] = df['Объем единицы, м3']
...
def show_quality_window():
    ...
    ctk.CTkButton(quality_window, text="Обработать без изменения данных", command=countinue_without_changes).pack(
        pady=5)
    ctk.CTkButton(quality_window, text="Улучшить качество данных и обработать", command=improve_data_quality).pack(
        pady=5)
